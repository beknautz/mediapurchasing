<?php
/**
 * portal/actions/request-revision.php
 * Client portal action — request revisions via review token.
...
if ($revisionNotes === '') {
    http_response_code(400);
    echo json_encode(['success' => false, 'error' => 'Please provide revision notes.']);
    exit;
}
...
try {
    ...
    try {
        ...
        error_log('[AI Video Portal] revision notify error: ' . $e->getMessage());
    }

    echo json_encode(['success' => true, 'campaign_id' => $campaignId]);

} catch (Throwable $e) {
    error_log('[AI Video Portal] request-revision.php error: ' . $e->getMessage());
