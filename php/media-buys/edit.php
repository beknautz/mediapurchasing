...
// Non-admin buyers can only edit their own buys
if (!$isAdmin && (int)($buy['buyer_id'] ?? 0) !== $userId) {
    $_SESSION['flash'] = ['type' => 'danger', 'message' => 'You do not have permission to edit this media buy.'];
...
$formData = [
    ...
    'internal_notes' => $buy['internal_notes'] ?? '',
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        if ($saveResult['success']) {
            $_SESSION['flash'] = ['type' => 'success', 'message' => 'Media buy updated successfully.'];
            redirect('/media-buys/view.php?id=' . $id);
        } else {
...
$pageTitle = 'Edit Media Buy — ' . ($formData['title'] ?: 'Untitled');
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-0 fw-bold">
            <i class="bi bi-pencil-square me-2 text-primary"></i>Edit Media Buy
        </h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 small">
                <li class="breadcrumb-item"><a href="/dashboard.php">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="/media-buys/index.php">Media Buys</a></li>
                <li class="breadcrumb-item"><a href="/media-buys/view.php?id=<?= $id ?>"><?= h($buy['title']) ?></a></li>
                <li class="breadcrumb-item active">Edit</li>
            </ol>
        </nav>
    </div>
    <a href="/media-buys/view.php?id=<?= $id ?>" class="btn btn-outline-secondary">
...
<div class="row g-4">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body">
                ...
                <div class="mt-3">
                    ...
                    <textarea class="form-control" id="internal_notes" name="internal_notes" rows="2"><?= h($formData['internal_notes']) ?></textarea>
                ...
                <div class="table-responsive">
                    <table class="table align-middle mb-0" id="lineItemsTable">
                        ...
                        <?php if (!empty($items)): ?>
                            ...
                            <tr class="line-item-row">
                                ...
                                <td>
                                    <input type="text" class="form-control form-control-sm"
                                           name="item_placement[]"
                                           value="<?= h($item['placement'] ?? $item['notes'] ?? '') ?>"
                                           placeholder="Placement / daypart">
                                </td>
                                <td>
                                    <input type="number" class="form-control form-control-sm item-spots"
                                           name="item_spots[]"
                                           value="<?= h($item['quantity'] ?? 1) ?>" min="0" step="1">
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="input-group input-group-sm">
                                        ...
                                        <input type="number" class="form-control item-total-cost"
                                               ...
                                               value="<?= number_format((float)($item['total_cost'] ?? 0), 2) ?>"
                        ...
                        <?php else: ?>
                            <tr class="line-item-row">
                                <td class="ps-3">
                                    <input type="text" class="form-control form-control-sm" name="item_description[]" placeholder="Description">
                                </td>
                                <td>
                                    <input type="text" class="form-control form-control-sm" name="item_placement[]" placeholder="Placement / daypart">
                                </td>
                                <td>
                                    <input type="number" class="form-control form-control-sm item-spots" name="item_spots[]" value="1" min="0" step="1">
                                </td>
                                <td>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">$</span>
                                        <input type="number" class="form-control item-unit-cost" name="item_unit_cost[]" value="0.00" min="0" step="0.01">
                                    </div>
                                </td>
                                <td>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">$</span>
                                        <input type="number" class="form-control item-total-cost" name="item_total_cost[]" value="0.00" min="0" step="0.01" readonly>
                                    </div>
                                </td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-outline-danger remove-line-item">
                        ...
                        <tfoot>
                            <tr class="table-light">
                                ...
                                <td>
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text">$</span>
                                        <input type="text" class="form-control fw-bold bg-white" id="grandTotal" value="0.00" readonly>
                                    </div>
                                </td>
                                <td></td>
                            </tr>
    ...
    <div class="col-lg-4">
        <div class="card border-0 shadow-sm sticky-top" style="top:1rem;">
            <div class="card-header bg-white py-3">
                <h5 class="mb-0 fw-semibold"><i class="bi bi-floppy me-2 text-primary"></i>Save Changes</h5>
            </div>
            <div class="card-body">
                <div class="mb-3 p-2 bg-light rounded small">
                    ...
                    <?php
                    $statusColors = [
                        'draft' => 'secondary', 'sent_to_vendor' => 'info',
                        'pending_client_approval' => 'warning', 'negotiating' => 'primary',
                        'client_approved' => 'success', 'finalized' => 'dark', 'cancelled' => 'danger',
                    ];
                    $statusLabels = [
                        'draft' => 'Draft', 'sent_to_vendor' => 'Sent to Vendor',
                        'pending_client_approval' => 'Pending Approval', 'negotiating' => 'Negotiating',
                        'client_approved' => 'Client Approved', 'finalized' => 'Finalized', 'cancelled' => 'Cancelled',
                    ];
                    $curStatus = $buy['status'] ?? 'draft';
                    $curColor  = $statusColors[$curStatus] ?? 'secondary';
                    $curLabel  = $statusLabels[$curStatus] ?? ucfirst($curStatus);
                    ?>
                    <span class="badge bg-<?= $curColor ?> fs-6"><?= h($curLabel) ?></span>
                </div>
                <div class="d-grid gap-2">
                    <button type="submit" name="save_draft" class="btn btn-primary btn-lg">
                        <i class="bi bi-floppy me-2"></i>Save Changes
                    </button>
                </div>
            </div>
            <div class="card-footer bg-white text-center">
                <a href="/media-buys/view.php?id=<?= $id ?>" class="text-muted small">
...
(function () {
    ...
    function attachRowListeners(row) {
        row.querySelector('.item-spots').addEventListener('input', function () { calcRow(row); calcGrandTotal(); });
        row.querySelector('.item-unit-cost').addEventListener('input', function () { calcRow(row); calcGrandTotal(); });
    ...
    document.getElementById('addLineItem').addEventListener('click', function () {
        ...
        template.querySelectorAll('input').forEach(function (inp) {
            if (inp.classList.contains('item-spots')) { inp.value = '1'; }
            else if (inp.type === 'number') { inp.value = '0.00'; }
            else { inp.value = ''; }
        });
